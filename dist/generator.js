...
export async function generateProject(config) {
    ...
    if (!existsSync(commonDir))
        throw new Error('Could not find template folder for, ' + commonDir);
    const baseLanguageDir = path.resolve(__dirname, `../templates/${language}`);
    if (!existsSync(baseLanguageDir))
        throw new Error('Could not find template folder for, ' + baseLanguageDir);
    const modelDir = path.resolve(__dirname, `../templates/models/${model}/${language}`);
    if (!existsSync(modelDir))
        throw new Error('Could not find template folder for, ' + modelDir);
    const prettierDir = path.resolve(__dirname, `../templates/prettier/${language}`);
    if (!existsSync(prettierDir))
        throw new Error('Could not find template folder for, ' + prettierDir);
    const eslintDir = path.resolve(__dirname, `../templates/eslint/${language}/${eslintConfig}`);
    if (!existsSync(eslintDir))
        throw new Error('Could not find template folder for, ' + eslintDir);
